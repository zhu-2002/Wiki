...
export default defineComponent({
  ...
  setup(){
     ...
     */
    const categorys = ref();
    const categorysTree = ref(); // 一级分类树，children属性就是二级分类
    ...
    const handleQuery = () => {
      ...
      axios.get("/category/list", {
        ...
        if ( data.success ){
          ...
          console.log("树形结构：", categorysTree);
        }
        else {
    ...
    const dectQuery = () => {
      ...
      axios.get("/category/list", {
        ...
        const data = response.data;
        if ( data.success ){
          categorysTree.value = data.content ;
    ...
    const remove = (id: number) => {
      axios.delete("/category/del/"+id).then((response) => {
        const data = response.data; // data = commonResp
        if (data.success) {
          // 重新加载列表
          handleQuery();
